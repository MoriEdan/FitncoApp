...
export const AdminMessageDetailScreen = props => {
  ...
  const getMessages = async (token, page) => {
    ...
    axios
      .get(MESSAGES + '?client=' + props.route.params.id, {
      ...
      .then(res => {
        ...
        if (!user) {
          setUserId(res.data.data.client.id);
          setUser(res.data.data.client);
        }
        setLoading(false);
      })
      .catch(e => {
  ...
  const getMedia = async page => {
    ...
    axios
      .get(ADMIN_MESSAGES_MEDIA + props.route.params.id, {
        headers: {Authorization: 'Bearer ' + token},
  ...
  const setPhoto = async res => {
    if (res?.didCancel) {
      ...
      data.append('to', user?.id);
      data.append('log', 'none');
      data.append('file', {
        type: photo.type,
        name: photo.fileName,
        uri: photo.uri,
    ...
    <Container>
      <KeyboardAvoidingView
        ...
        <View style={styles.header}>
          ...
          <Image source={{uri: user?.picture}} style={styles.logoImage} />
